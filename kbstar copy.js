...
빠른조회.prototype.빠른잔액조회 = function(aInput) { // Quick Balance Inquiry
    ...
    try {
       ...
       // =========================================================
        ...
        // 예금명 // Deposit name
        this.iSASInOut.Output.Result.예금명 = StrGrab(StrGrab(ResultStr, ">계좌정보<", "</tr>"), "<td", "</td>"); // Account information
...
빠른조회.prototype.빠른거래내역조회 = function(aInput) { // Quick Inquiry Quick Transaction History Inquiry
    this.log(BankName + " 빠른조회 빠른거래내역조회 호출 [" + aInput + "][" + moduleVersion + "]"); // Quick Inquiry Quick Transaction History Inquiry Call
    var 거래Index = 0;
    try {
        system.setStatus(IBXSTATE_CHECKPARAM, 10);

        var input = dec(aInput.Input);
        // if (input.계좌비밀번호) this.iSASInOut.Input.계좌비밀번호 = input.계좌비밀번호.replace(/./g, '*'); // Password
        // if (input.주민사업자번호) this.iSASInOut.Input.주민사업자번호 = input.주민사업자번호.toString().replace(/./g, "*"); // Resident business registration number

        var 계좌번호 = input.계좌번호; // Account number
        var 계좌비밀번호 = input.계좌비밀번호; // Password
        var 조회시작일 = input.조회시작일; // Start date of inquiry
        var 조회종료일 = input.조회종료일;// End date of inquiry
        var ID = input.ID;// Customer ID
        var 주민사업자번호 = input.주민사업자번호;// Resident business registration number

        var 통화 = input.통화 && input.통화.toUpperCase();
        if (!통화 || 통화 == 'KRW') 통화 = "";

        // 미입력 체크 // Check for missing input
        if (!ID && !주민사업자번호) { // No input check
            this.setError(E_IBX_USER_ACCOUNT_NOTENTER);
            return E_IBX_USER_ACCOUNT_NOTENTER;
        }
        ...
        // 계좌번호 확인 // Account number verification
        if (계좌번호 == "" || 계좌번호 == undefined) {
            this.setError(E_IBX_ACCOUNT_NO_NOTENTER);
            return E_IBX_ACCOUNT_NO_NOTENTER;
        }
        ...
        // 계좌비밀번호 확인 // Password verification
        if (계좌비밀번호 == "" || 계좌비밀번호 == undefined) {
            this.setError(E_IBX_ACCOUNT_PASSWORD_NOTENTER);
            return E_IBX_ACCOUNT_PASSWORD_NOTENTER;
        }

        // 비밀번호는 4자리 숫자여야 함 // Password must be 4 digits
        if (!/^\d{4}$/.test(계좌비밀번호)) {
            this.setError(E_IBX_ACCOUNT_PASSWORD_INVALID);
            return E_IBX_ACCOUNT_PASSWORD_INVALID;
        }

        // 날짜 확인 // Date verification
        if (조회시작일 == "") {
            this.setError(E_IBX_ENUM_DATE_BEGIN_NOTENTER);
            return E_IBX_ENUM_DATE_BEGIN_NOTENTER;
        }

        // 조회 시작일 // Start date of inquiry
        if (!/^\d{8}$/.test(조회시작일)) {
            this.setError(E_IBX_ENUM_DATE_BEGIN_INVALID);
            return E_IBX_ENUM_DATE_BEGIN_INVALID;
        }
        // 조회 시작일 유효성 검사 // Validation of inquiry start date
        ...
        var 육개월전 = new Date();
        육개월전.setMonth(육개월전.getMonth() - 6);// 6 months ago
        ...
        if (ResultStr.indexOf("사용자가 많아 잠시 후 이용하여") >= 0) { // // There are many users, so please use it after a while
            this.setError(E_IBX_SITE_INTERNAL);
            return E_IBX_SITE_INTERNAL;
        }
        ...
        // Main > 계좌조회 // Account inquiry
        system.setStatus(IBXSTATE_LOGIN, 40);

        // When entering a resident business registration number, search by resident business registration number
        this.postData = "KEYPAD_TYPE_" + tagName + "=" + StrGrab(ResultStr, "name='KEYPAD_TYPE_" + tagName + "' value=\"", "\"");
        this.postData += "&KEYPAD_HASH_" + tagName + "=";
        this.postData += "&KEYPAD_USEYN_" + tagName + "=" + StrGrab(ResultStr, "name='KEYPAD_USEYN_" + tagName + "' value=\"", "\"");
        this.postData += "&KEYPAD_INPUT_" + tagName + "=" + httpRequest.URLEncode(StrGrab(ResultStr, "name='KEYPAD_INPUT_" + tagName + "' value=\"", "\""), "UTF-8");
        ...
        if (통화 && !is외화) { // 통화 입력 시 외화만 지원 // Only foreign currency is supported when entering currency
            this.setError(E_IBX_CURRENCY_INVALID);
            return E_IBX_CURRENCY_INVALID;
        }
        ...
        if (통화) {
            while (true) {
                // 계좌조회 페이지 // Account inquiry page
                system.setStatus(IBXSTATE_RESULT, 70);

                // 헤더 오류체크 // Header error check
                ...
                while (true) {
                    ...
                    // 출금액, 입금액 // Withdrawal amount, deposit amount
                    ...
                    // 거래후잔액 // Balance after transaction
                    ...
                    // 잔액이 없는 통화 입력 시 800032B1(유효하지 않은 통화) // In case of entering a currency with no balance, 800032B1 (invalid currency)
                    if (!items.거래후잔액) {
                        this.setError(E_IBX_CURRENCY_INVALID);
                        return E_IBX_CURRENCY_INVALID;
                    }
            ...
            // 통화 입력 오류 검증 // Currency input error verification
            if (!is통화) {
                this.setError(E_IBX_CURRENCY_INVALID);
                return E_IBX_CURRENCY_INVALID;
            }
            ...
            while (true) {
                ...
                while (true) {
                    ...
                    // 기재사항1 // Note 1
                    // 통장표시내용 항목에 값이 있으면 통장표시내용으로 결과 처리, 없으면 의뢰인/수취인 항목으로 결과 처리 // If there is a value in the passbook display content item, process the result as passbook display content, otherwise process the result as requester/recipient item
        ...
        this.iSASInOut.Output.ErrorMessage = "";
        this.iSASInOut.Output.Result = {};
        this.iSASInOut.Output.Result.내역정렬순서 = 내역정렬순서; // 0 최근거래순, 1 과거거래순 // 0 recent transaction order, 1 past transaction order
        ...
        system.setStatus(IBXSTATE_DONE, 100);
        this.log(BankName + " Quick Inquiry Quick Transaction History Inquiry finally");
    }
};
///////////////////////////////////////////////////////////////////////////////////////////
//include 등등 필요한거 설정.
